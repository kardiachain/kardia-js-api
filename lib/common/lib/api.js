...
var _nat2 = _interopRequireDefault(_nat);
...
exports.default = function (provider) {
  ...
  var encodeABI = function encodeABI(type, value) {
    if (type === 'bytes') {
      var length = _bytes2.default.length(value);
      var nextMul32 = (((length - 1) / 32 | 0) + 1) * 32;
      var lengthEncoded = encodeABI('uint256', _nat2.default.fromNumber(length)).data;
      var bytesEncoded = _bytes2.default.padRight(nextMul32, value);
      return { data: _bytes2.default.concat(lengthEncoded, bytesEncoded), dynamic: true };
    } else if (type === 'uint256' || type === 'bytes32' || type === 'address') {
      return { data: _bytes2.default.pad(32, value), dynamic: false };
    } else {
  ...
  var removeEmptyTo = function removeEmptyTo(tx) {
    return tx.to === '' || tx.to === '0x' ? _map2.default.remove('to')(tx) : tx;
  };

  var waitTransactionReceipt = getTransactionReceipt; // TODO: implement correctly

  var addTransactionDefaults = function () {
    var _ref6 = (0, _asyncToGenerator3.default)( /*#__PURE__*/_regenerator2.default.mark(function _callee(tx) {
      var baseDefaults, _ref, _ref2, chainIdNum, gasPrice, nonce, value, data, chainId, from, to, gas;

      return _regenerator2.default.wrap(function _callee$(_context) {
        while (1) {
          switch (_context.prev = _context.next) {
            case 0:
              baseDefaults = [tx.chainId || send('net_version')(), tx.gasPrice || send('eth_gasPrice')(), tx.nonce || send('eth_getTransactionCount')(tx.from, 'latest'), tx.value || '0x0', tx.data || '0x'];
              _context.next = 3;
              return _promise2.default.all(baseDefaults);
            ...
            case 3:
              ...
              value = _ref2[3];
              data = _ref2[4];
              chainId = _nat2.default.fromNumber(chainIdNum);
              from = tx.from;
              to = tx.to === '' || tx.to === '0x' ? null : tx.to;
            ...
            case 17:
              _context.t1 = _nat2.default;
              _context.t2 = _nat2.default;
              _context.next = 21;
              return send('eth_estimateGas')({
                from: tx.from,
                to: tx.to,
                value: tx.value,
                nonce: tx.nonce,
            ...
            case 21:
              ...
              _context.t4 = _context.t2.mul.call(_context.t2, _context.t3, '0x6');
              _context.t0 = _context.t1.div.call(_context.t1, _context.t4, '0x5');
    ...
    return function addTransactionDefaults(_x) {
      return _ref6.apply(this, arguments);
    };
  }();

  var sendTransactionWithDefaults = function () {
    var _ref7 = (0, _asyncToGenerator3.default)( /*#__PURE__*/_regenerator2.default.mark(function _callee2(tx) {
      return _regenerator2.default.wrap(function _callee2$(_context2) {
        while (1) {
          switch (_context2.prev = _context2.next) {
            case 0:
              _context2.t0 = sendTransaction;
              _context2.t1 = removeEmptyTo;
              _context2.next = 4;
              return addTransactionDefaults(tx);
    ...
    return function sendTransactionWithDefaults(_x2) {
  ...
  var callMethodData = function callMethodData(method) {
    return function () {
      for (var _len2 = _arguments.length, params = Array(_len2), _key2 = 0; _key2 < _len2; _key2++) {
        params[_key2] = _arguments[_key2];
      }

      var methodSig = method.name + '(' + method.inputs.map(function (i) {
        return i.type;
      }).join(',') + ')';
      var methodHash = (0, _hash.keccak256s)(methodSig).slice(0, 10);
      ...
      for (var i = 0; i < encodedParams.length; ++i) {
        if (encodedParams[i].dynamic) {
          var dataLoc = encodedParams.length * 32 + _bytes2.default.length(dataBlock);
          headBlock = _bytes2.default.concat(headBlock, _bytes2.default.pad(32, _nat2.default.fromNumber(dataLoc)));
          dataBlock = _bytes2.default.concat(dataBlock, encodedParams[i].data);
        } else {
          headBlock = _bytes2.default.concat(headBlock, encodedParams[i].data);
        }
      }
      return _bytes2.default.flatten([methodHash, headBlock, dataBlock]);
    };
  };
      ...
      if (method && method.name) {
        var _call = function _call(type, value) {
          return function () {
            var transaction = {
              ...
              to: address,
              value: value,
              data: callMethodData(method).apply(undefined, _arguments)
            };
            return type === 'data' ? _promise2.default.resolve(transaction) : method.constant ? callWithDefaults(transaction) : sendTransactionWithDefaults(transaction).then(type === 'receipt' ? waitTransactionReceipt : function (x) {
              return x;
            });
          };
        };
        ...
        if (!method.constant) {
          ...
          contract[method.name + '_pay'] = function (value) {
            return function () {
              return _call('receipt', value).apply(undefined, _arguments);
            };
          };
          contract[method.name + '_pay_txHash'] = function (value) {
            return function () {
              return _call('txHash', value).apply(undefined, _arguments);
            };
          };
